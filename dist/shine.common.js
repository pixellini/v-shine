 ...
 * @date 16/06/2019
 */
var directive_shine_shine = function shine(el, binding) {
  var _ref = binding.value || {},
      ...
      shineWidth = _ref.shineWidth,
      fromLeft = _ref.fromLeft,
      extraShine = _ref.extraShine; // Constants
  ...
  var createContainer = function createContainer() {
    ...
    var styles = {
      ...
      width: '100%',
      overflow: 'hidden',
      filter: "blur(".concat(BLUR_AMOUNT, "px)")
    };
  ...
  var createShineElement = function createShineElement(size, width, shineAngle) {
    var element = document.createElement('div');
    var styles = {
      position: 'absolute',
      left: '50%',
      top: '-100%',
      transform: "rotate(".concat(-shineAngle, "deg) translate(-50%, -50%)"),
      transformOrigin: 'top left',
      backgroundColor: COLOR,
      height: width + 'px',
      // Not actually width, it's only because we are rotating it that it looks like it.
      width: size + 'px',
      opacity: OPACITY,
      transition: "all ".concat(TIME, "s ").concat(EASE)
    };
    Object.keys(styles).forEach(function (key) {
  ...
  var createExtraShine = function createExtraShine(props) {
    if (extraShine) {
      var container = props.container,
          size = props.size,
          top = props.top,
          width = props.width,
          shineAngle = props.shineAngle; // Extra shine element will always be a quarter of the size of the normal shine element.

      var element = createShineElement(size, width / 4, shineAngle);
      container.appendChild(element); // Animate extra shine.
  ...
  var shineAnimation = function shineAnimation(props) {
    // Deconstruct.
    var element = props.element,
        size = props.size,
        height = props.height,
        width = props.width,
        shineAngle = props.shineAngle; // Create elements.

    var container = createContainer();
    var shineElement = createShineElement(size, width, shineAngle);
    container.appendChild(shineElement);
    element.appendChild(container); // Calculate end position.

    var top = "calc(100% + ".concat(height + width, "px)");
    var shineDelay = DELAY * 1000; // Show the shine.

    setTimeout(function () {
      shineElement.style.top = top; // The extra shine behind the main shine.

      createExtraShine(_objectSpread({}, props, {
        top: top,
        container: container
      }));
    }, 40 + shineDelay); // Remove the container & shine elements from the DOM.
  ...
  var getShineAngle = function getShineAngle(height, width) {
  ...
  var onEventHandler = function onEventHandler(e) {
    ...
    isAnimating = mouseEntered = true; // Get main element and shine element details.

    var target = e.target;
    var width = target.clientWidth;
    var height = target.clientHeight;
    var size = Math.hypot(height, width) * 1.5; // Length of the shine bar.

    var shineAngle = getShineAngle(height, width); // Determine the height of the shine.
...
/* harmony default export */ var directive_shine = (directive_shine_shine);
...
/******/ });
//# sourceMappingURL=shine.common.js.map